...
import { glovesimg } from '../../Components/ProductList';
import glove1 from '../../image/gloves1.jpg';

const ProKeepingLeather = () => {

     const { addToCart } = useContext(CartContext);
         const [mainImage, setMainImage] = useState(glovesimg[0]);
            
                const product = {
                    id: 2002,
                    title: "Pro Keeping Leather Gloves",
                    mrp: 5000,
                    price: 4500,
                    img: mainImage,
      ...
      <div className='flex'>
                                  <div className="w-full max-w-md m-36">
                                      ...
                                      <div className="flex justify-between gap-4">
                                          {glovesimg.slice(1).map((img, index) => (
                                              <img
                                                  key={index}
                                                  src={img}
                                                  alt="Thumbnail"
                              ...
                              <div className='flex  gap-20 mt-20 mx-[450px]'>
                                  <Link to="/blackmamba"><img src={glove1} alt={product.title} className='w-36 gap-20 h-32 rounded-lg cursor-pointer hover:scale-105 transition' /></Link>
                                  <Link to="/singleblade"><img src={glove1} alt={product.title} className='w-36 gap-20 h-32 rounded-lg cursor-pointer hover:scale-105 transition' /></Link>
                                  <Link to="/doubleblade"><img src={glove1} alt={product.title} className='w-36 gap-20 h-32 rounded-lg cursor-pointer hover:scale-105 transition' /></Link>
                              </div>
...
export default ProKeepingLeather
