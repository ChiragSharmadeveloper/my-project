...
import { guardimages } from '../../Components/ProductList';
import guard1 from '../../image/guard1.webp';

const EliteEdGuard = () => {
    const { addToCart } = useContext(CartContext);
    const [mainImage, setMainImage] = useState(guardimages[0]);

    const product = {
        id: 2101,
        title: "Elite Edition Thigh Guard",
        mrp: 5000,
        price: 4500,
        img: mainImage,
    ...
    return (
        <>
            <div className='flex'>
                <div className="w-full max-w-md m-36">
                    ...
                    <div className="flex justify-between gap-4">
                        {guardimages.slice(1).map((img, index) => (
                            <img
                                key={index}
                                src={img}
                                alt="Thumbnail"
            ...
            <div className='flex  gap-20 mt-20 mx-[450px]'>
                <Link to="/blackmamba"><img src={guard1} alt={product.title} className='w-36 gap-20 h-32 rounded-lg cursor-pointer hover:scale-105 transition' /></Link>
                <Link to="/singleblade"><img src={guard1} alt={product.title} className='w-36 gap-20 h-32 rounded-lg cursor-pointer hover:scale-105 transition' /></Link>
                <Link to="/doubleblade"><img src={guard1} alt={product.title} className='w-36 gap-20 h-32 rounded-lg cursor-pointer hover:scale-105 transition' /></Link>
            </div>
        </>
    )
}

export default EliteEdGuard
